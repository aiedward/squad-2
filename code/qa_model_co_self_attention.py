...
from modules_co_self_attention import RNNEncoder, SimpleSoftmaxLayer, Coattention, BiLSTMEncoder, SelfAttention
...
class QAModel(object):
    ...
    def build_graph(self):
        ...
        self_attn_RNN_output = self_attention_layer.build_graph(coattn_RNN_output)

        biLSTM_encoder = BiLSTMEncoder(self.FLAGS.hidden_size, self.keep_prob, scope="biLSTM2")
        overall_output = biLSTM_encoder.build_graph(self_attn_RNN_output)

        # # Apply fully connected layer to each blended representation
        # Note, blended_reps_final corresponds to b' in the handout
        # Note, tf.contrib.layers.fully_connected applies a ReLU non-linarity here by default
        blended_reps_final = tf.contrib.layers.fully_connected(overall_output, num_outputs=self.FLAGS.hidden_size) # blended_reps_final is shape (batch_size, context_len, hidden_size)
